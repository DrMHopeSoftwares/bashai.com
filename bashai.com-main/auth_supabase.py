    ...
    def verify_password_supabase(self, password, email):
        ...
        try:
            ...
            response = requests.get(
                ...
                params={'email': f'eq.{email}', 'select': 'password_hash'}
            )
            
            if response.status_code == 200:
                users = response.json()
                if users and len(users) > 0:
                    password_hash = users[0].get('password_hash')
                    if password_hash:
    ...
    def change_password(self, user_id, new_password):
        ...
        try:
            ...
            response = requests.patch(
                ...
                headers=self.headers,
                params={'id': f'eq.{user_id}'},
                json={'password_hash': password_hash, 'updated_at': datetime.utcnow().isoformat()}
            )
            return response.status_code == 204
        except:
            return False
